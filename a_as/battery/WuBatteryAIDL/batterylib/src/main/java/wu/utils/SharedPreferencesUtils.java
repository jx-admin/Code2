...
public class SharedPreferencesUtils {
    private static final String BATTERY_SP_FILE = "battery.d";
    private static final String TARGET_PACKAGE_NAME = BuildConfig.APPLICATION_ID;// 目标数据程序的包名，
    ...
    public static <T extends Serializable> boolean put(Context context, String key, T value) {
        if (key != null && key.length() > 0) {
            SharedPreferences sp = context.getSharedPreferences(BATTERY_SP_FILE, Context.MODE_WORLD_READABLE | Context.MODE_MULTI_PROCESS);
            return put(sp, key, value);
        }
        return false;
    }
