 ...
 * 在Menifest.xml里配置例如
 * 《provider
 *             android:name="包名.DBProvider"
 *             android:authorities="com.act.sctc.provider" 》
 * 重写CONTENT_ITEM_TYPE & CONTENT_ITEM
 * </pre>
 * 
 */
public class DBProvider extends ContentProvider {
	...
	// 定义MIME类型，访问单个记录
	public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.provider.creditcard";
	// 访问数据集
	public static final String CONTENT_ITEM = "vnd.android.cursor.dir/vnd.providers.creditcard";
	...
	public Cursor query(Uri uri, String[] columns, String selection,
		...
		switch (uri.getPathSegments().size()) {
		case 1:
			c = db.query(uri.getLastPathSegment(), columns, selection,
					selectionArgs, null, null, orderBy);
			break;
		case 2:
			c = db.query(uri.getPathSegments().get(0), columns,
					BaseColumns._ID
							+ "="
		...
		return c;
		/*
		 * SQLiteQueryBuilder qb = new SQLiteQueryBuilder(); switch
		 * (uri.getPathSegments().size()) { // 查询所有 case PRODUCT_PHONE:
		 * qb.setTables(uri.getLastPathSegment());
		 * qb.setProjectionMap(callProjectionMap); break; // 根据ID查询 case
		 * PRODUCT_PHONE_ID: qb.setTables(uri.getLastPathSegment());
		 * qb.setProjectionMap(callProjectionMap);
		 * qb.appendWhere(BaseColumns._ID + "=" + uri.getPathSegments().get(1));
		 * break; default: throw new IllegalArgumentException("Uri错误！ " + uri);
		 * }
		 * 
		 * // 返回游标集合 Cursor c = qb.query(db, columns, selection, selectionArgs,
		 * null, null, orderBy); c.setNotificationUri(contentResolver, uri);
		 * return c;
		 */
	}
