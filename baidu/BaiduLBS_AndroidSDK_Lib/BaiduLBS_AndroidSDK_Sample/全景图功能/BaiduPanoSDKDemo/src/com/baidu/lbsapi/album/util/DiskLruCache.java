...
public class DiskLruCache {
    ...
    private final int maxCacheItemSize = 682; // 64 item default
    private long maxCacheByteSize = 1024 * 1024 * 2; // 5MB default
    ...
    public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        ...
        if (cacheDir.isDirectory() && cacheDir.canWrite() && StreetscapeUtil.getUsableSpace(cacheDir) > maxByteSize) {
     ...
     * {@link com.baidu.demo.util.DiskLruCache#openCache(android.content.Context, java.io.File, long)}
        ...
        synchronized (mLinkedHashMap) {
            if (mLinkedHashMap.get(key) == null) {
                try {
                    ...
                    //Log.e("Demo", "Error in put: " + e.getMessage());
    ...
    public Bitmap get(String key) {
        synchronized (mLinkedHashMap) {
            ...
            if (file != null) {
                ...
                return ImageLoader.getInstance().getButmapDecoder().decodeSampledBitmapFromFile(file);
            } else {
                final String existingFile = createFilePath(mCacheDir, key);
                if (new File(existingFile).exists()) {
                    put(key, existingFile);
                    return ImageLoader.getInstance().getButmapDecoder().decodeSampledBitmapFromFile(existingFile);
                }
            }
            return null;
        }
     ...
     * {@link com.baidu.demo.util.DiskLruCache#clearCache(android.content.Context, String)}
     * or {@link com.baidu.demo.util.DiskLruCache#clearCache()} instead.
     *
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath = Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED
                || !StreetscapeUtil.isExternalStorageRemovable() ? StreetscapeUtil.getExternalCacheDir(context)
     ...
     * {@link com.baidu.demo.util.DiskLruCache#setCompressParams(android.graphics.Bitmap.CompressFormat, int)}
    ...
    private boolean writeBitmapToFile(Bitmap bitmap, String file) throws IOException, FileNotFoundException {
        ...
        try {
            out = new BufferedOutputStream(new FileOutputStream(file), StreetscapeUtil.IO_BUFFER_SIZE);
