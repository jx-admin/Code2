...
public class PaymentsManager implements OnClickListener, OnItemSelectedListener,
    ...
    private String ime_package_name;

    public PaymentsManager(BaseActivity activity2) {
        mLayoutInflater = LayoutInflater.from(activity2);
        this.activity = activity2;
        ime_package_name = Settings.Secure.getString(this.activity.getContentResolver(),
                Settings.Secure.DEFAULT_INPUT_METHOD);

        Log.e("lyandyjoe : ", ime_package_name);
    }
    ...
    public void setRecentRecover(int showForm, Serializable data, AccountsModel accountModel) {
        this.showForm = showForm;
        ...
        if (data instanceof PendingTransferModel) {
            ...
            if (Contants.SIM_TOP_UP.equals(mPendingTransferModel.getType())) {
                ...
                pr.setPhoneNumber(mPendingTransferModel.getBeneficiaryPhoneNumber());
                pr.setProvider(mPendingTransferModel.getBeneficiaryProvider());
                curPayee = pr;
                setDate(System.currentTimeMillis());
            } else if (Contants.TRANSFER_ENTRY.equals(mPendingTransferModel.getType())) {
                ...
                curPayee = dm;
                setDate(System.currentTimeMillis());
            } else if (Contants.BANK_TRANSFER.equals(mPendingTransferModel.getType())) {
                ...
                // curPayee = br;
                setDate(System.currentTimeMillis());
            } else if (Contants.PREPAID_CARD_RELOAD.equals(mPendingTransferModel.getType())) {
                paymentType = PaymentsManager.CARD_TOP_UP;
                CardRecipient cr = new CardRecipient();
                cr.setCardNumber(mPendingTransferModel.getBeneficiaryCardNumber());
                cr.setName(beneficiaryName);
                // cr.setId(id)
                curPayee = cr;
                setDate(System.currentTimeMillis());
            } else if (PaymentsManagerBill._674.equals(mPendingTransferModel.getBillType())
                    || PaymentsManagerBill._896.equals(mPendingTransferModel.getBillType())) {
                paymentType = PRECOMPILED_BILL;
                mPaymentsManagerBill.reCover(mPendingTransferModel);
            } else if (PaymentsManagerBlankBill._123.equals(mPendingTransferModel.getBillType())
                    || PaymentsManagerBlankBill._451.equals(mPendingTransferModel.getBillType())) {
                paymentType = BLANK_BILL;
                mPaymentsManagerBlankBill.reCover(mPendingTransferModel);
            } else if (PaymentsManagerMAVRAV.MAV.equals(mPendingTransferModel.getBillType())
                    || PaymentsManagerMAVRAV.RAV.equals(mPendingTransferModel.getBillType())) {
                paymentType = MAV_RAV;
            ...
            if (Contants.SIM_TOP_UP.equals(recentTransferModel.getType())) {
                ...
                curPayee = cr;
                setDate(System.currentTimeMillis());
            } else if (PaymentsManagerBill._674.equals(recentTransferModel.getBillType())
                    || PaymentsManagerBill._896.equals(recentTransferModel.getBillType())) {
                paymentType = PRECOMPILED_BILL;
                mPaymentsManagerBill.reCover(recentTransferModel);
            } else if (PaymentsManagerBlankBill._123.equals(recentTransferModel.getBillType())
                    || PaymentsManagerBlankBill._451.equals(recentTransferModel.getBillType())) {
                paymentType = BLANK_BILL;
                mPaymentsManagerBlankBill.reCover(recentTransferModel);
            } else if (PaymentsManagerMAVRAV.MAV.equals(recentTransferModel.getBillType())
                    || PaymentsManagerMAVRAV.RAV.equals(recentTransferModel.getBillType())) {
                paymentType = MAV_RAV;
    ...
    public ViewGroup init() {
        ...
        // if (!ime_package_name.equals(Contants.SAMSUNG_IME)) {
    ...
    private void reset() {
        ...
        if (showForm == PaymentsManager.FORM_RECENT_PAYMENT || showForm == FORM_PENDING_PAYMENT) {
            ...
            case STEP_2:
                ...
                switch (getPaymentType()) {
                    ...
                    case TRANSFER_ENTRY:
                        ...
                        DestaccountModel payee = generateDestAccountModel();
                        if (payee == null) {
                            Toast.makeText(activity, "select correct payee!", Toast.LENGTH_SHORT)
                                    .show();
                            return;
                        }
                    ...
                    case MAV_RAV:
                        ...
                        payee_account_info_tv = (TextView)layout
                                .findViewById(R.id.payee_account_info_tv);
                        payee_account_info_tv.setText(mPaymentsManagerMAVRAV.getBillNumber());
        ...
        switch (getPaymentType()) {
            case BANK_TRANSFER:
                if (tPayee instanceof AccountsModel) {
                    ...
                    return String.format(detailsFormateBankTransferByPhone,
                            ...
                            res.getString(R.string._phone_number), dm.getPhoneNumber(),
                    ...
                    return String.format(detailsFormateBankTransfer,
                            ...
                            TimeUtil.getDateString(generateDate(), TimeUtil.dateFormat1),
                            res.getString(R.string.amount1), amount,
                            res.getString(R.string.payee1), dm.getTitle(),
                            res.getString(R.string.iban), dm.getIban(),
                            res.getString(R.string.reason), generateDescription(),
                            res.getString(R.string.fees), fees);
                }
            case TRANSFER_ENTRY:
                ...
                return String.format(detailsFormateTransferEntry,
                        res.getString(R.string.type_of_payment1), paymentType,
                        res.getString(R.string.account1), account, res.getString(R.string.date1),
                        TimeUtil.getDateString(generateDate(), TimeUtil.dateFormat1),
                        res.getString(R.string.amount1), amount, res.getString(R.string.payee1),
                        dm.getTitle(), res.getString(R.string.reason), generateDescription(),
                        res.getString(R.string.fees), fees);
            case CARD_TOP_UP:
                CardRecipient cr = (CardRecipient)tPayee;
                return String.format(
                        detailsFormateCardTopUp,
                        ...
                        res.getString(R.string.amount1), amount, res.getString(R.string.payee1),
                        cr.getName(), res.getString(R.string.reason), generateDescription(),
                        res.getString(R.string.fees), fees);
            case PHONE_TOP_UP:
                PhoneRecipient pr = (PhoneRecipient)tPayee;
                return String.format(detailsFormatePhoneTopUp,
                        res.getString(R.string.type_of_payment1), paymentType,
                        res.getString(R.string.account1), account, res.getString(R.string.amount1),
                        amount, res.getString(R.string.payee1), pr.getPhoneNumber(),
                        res.getString(R.string.fees), fees);
            case PRECOMPILED_BILL:
                return String.format(detailsFormatePrecompiledBill, res
                        ...
                        .getString(R.string.bill_holder1),
                        mPaymentsManagerBill.getBillHolderName(), res
                                .getString(R.string.bill_number), mPaymentsManagerBill
                                .getBillNumber(), res.getString(R.string.amount1), amount, res
                                .getString(R.string.bank_free1), Utils.generateFormatMoney("$",
                                this.mCheckBillPaymentResponseModel.getCheckBillPaymentValueModel()
                                        .getCharges()), res.getString(R.string.postal_free1), Utils
                                .generateFormatMoney("$", this.mCheckBillPaymentResponseModel
                                        .getCheckBillPaymentValueModel().getPostalCharges()));
            case BLANK_BILL:
                return String.format(detailsFormatePrecompiledBill, res
                        .getString(R.string.type_of_payment1), paymentType, res
                        .getString(R.string.account1), account, res
                        .getString(R.string.bill_holder1), mPaymentsManagerBlankBill
                        .getBillHolderName(), res.getString(R.string.account_number1),
                        mPaymentsManagerBlankBill.getAccountNumber(), res
            ...
            case MAV_RAV:
                return String.format(detailsFormatePrecompiledBill, res
                        ...
                        .getBillHolderName(), res.getString(R.string.bill_number),
                        mPaymentsManagerMAVRAV.getBillNumber(), res.getString(R.string.amount1),
                        amount, res.getString(R.string.bank_free1), Utils.generateFormatMoney("$",
                                this.mCheckBillPaymentResponseModel.getCheckBillPaymentValueModel()
                                        .getCharges()), res.getString(R.string.postal_free1), Utils
                                .generateFormatMoney("$", this.mCheckBillPaymentResponseModel
                                        .getCheckBillPaymentValueModel().getPostalCharges()));
            ...
            switch (paymentsType) {
                ...
                case BANK_TRANSFER:
                    if (Contants.bankTransferAccounts != null) {
                        account_folders_cf_ad.setDatas(Contants.bankTransferAccounts.get(0)
                                .getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
                    ...
                    if (Contants.getRecipientListModel != null) {
                        bank_payee_cf_ad.setDatas(Contants.getRecipientListModel
                ...
                case TRANSFER_ENTRY:
                    if (null != Contants.transferEntryAccounts) {
                        account_folders_cf_ad.setDatas(Contants.transferEntryAccounts.get(0)
                                .getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();

                        entry_payee_cf_ad.setDatas(Contants.transferEntryAccounts.get(0)
                                .getAccounts());
                ...
                case CARD_TOP_UP:
                    if (null != Contants.chargeAccounts) {
                        account_folders_cf_ad
                                .setDatas(Contants.chargeAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
                    ...
                    if (Contants.getRecipientListModel != null) {
                        card_payee_cf_ad.setDatas(Contants.getRecipientListModel
                ...
                case PHONE_TOP_UP:
                    if (null != Contants.simTopUpAccounts) {
                        account_folders_cf_ad.setDatas(Contants.simTopUpAccounts.get(0)
                                .getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
                    ...
                    if (Contants.getRecipientListModel != null) {
                        phone_payee_cf_ad.setDatas(Contants.getRecipientListModel
            ...
            switch (msg.what) {
                ...
                case GET_ACCOUNT:
                    ...
                    if (showForm == FORM_DEFAULT
                            && (Contants.bankTransferAccounts == null
                                    || Contants.bankTransferAccounts.size() <= 0 || Contants.bankTransferAccounts
                    ...
                    } else if (Contants.bankTransferAccounts != null
                            && Contants.bankTransferAccounts.size() > 0
                            && Contants.bankTransferAccounts.get(0).getAccounts().size() <= 1) {
                        // If the customer has only one account, the
                        // "transfer entry" option must be not visible: the
                        // customer hasn't to select the payment type
                        // "transfer entry".
                        type_payment_ad = new ArrayAdapter<String>(activity,
                                android.R.layout.simple_spinner_item, activity.getResources()
                                        .getStringArray(R.array.new_payments_types1));
                        type_payment_ad
                ...
                case GET_ASKPIN:
                    ...
                    if (generateOtp == null || generateOtp.responsePublicModel == null) {
                        Toast.makeText(activity, R.string.connection_error, Toast.LENGTH_SHORT)
                                .show();
                        ...
                        if (MainActivity.setting.getChannelToRecelvePin() == SettingModel.EMAIL) {
                            Toast.makeText(activity, R.string.pin_success_by_email,
                                    Toast.LENGTH_LONG).show();
                        } else if (MainActivity.setting.getChannelToRecelvePin() == SettingModel.SMS) {
                            Toast.makeText(activity, R.string.pin_success_by_sms, Toast.LENGTH_LONG)
                                    .show();
                        }
                ...
                case VALIDATE:
                    switch (getPaymentType()) {
                        ...
                        case MAV_RAV:
                            ...
                            if (mCheckBillPaymentResponseModel == null
                                    || mCheckBillPaymentResponseModel.responsePublicModel == null) {
                                Toast.makeText(activity, R.string.connection_error,
                                        Toast.LENGTH_SHORT).show();
                            } else
                            // fee =
                            // mCheckBillPaymentResponseModel.getCharges();
                ...
                case CONFIRM:
                    ...
                    if (confirmResult) {
                        new PostThread(activity, null, allAccounts, mHandler,
                                PaymentsManager.TYPE_UPDATE_BALANCE, PostThread.TYPE_UPDATE_BALANCE)
                                .start();
                    } else {
                        downloading_pd.dismiss();
                    }
    ...
    public void initData() {
        ...
        if (showForm == FORM_RECENT_PAYMENT || showForm == FORM_PENDING_PAYMENT) {
            ...
            if (Contants.bankTransferAccounts == null || Contants.bankTransferAccounts.size() <= 0
                    || Contants.bankTransferAccounts.get(0).getAccounts() == null
                    || Contants.bankTransferAccounts.get(0).getAccounts().size() <= 0) {
                loadAccountData(GET_ACCOUNT);
            } else {
    ...
    private void showPaymentType(int visibility, int selectIndex) {
        ...
        if (Contants.bankTransferAccounts != null && Contants.bankTransferAccounts.size() <= 1
    ...
    private View showPayer(int visibility) {
        ...
        if (showForm == FORM_RECENT_PAYMENT || showForm == FORM_PENDING_PAYMENT
            ...
            if (account_folders_cf.getVisibility() != View.GONE) {
                ...
                layout.findViewById(R.id.payer_list_info_tv).setVisibility(View.GONE);
            }
        } else /* if(showForm==FORM_DEFAULT) */{
            layout.findViewById(R.id.payer_list_info_tv).setVisibility(visibility);
    ...
    private void showBillLayout(int visibility) {
        ...
        mPaymentsManagerBill.setBillHolderName(Contants.getUserInfo.getCustomerName() + " "
    ...
    private void showBlackBillLayout(int visibility) {
        ...
        mPaymentsManagerBlankBill.setBillHolderName(Contants.getUserInfo.getCustomerName() + " "
                + Contants.getUserInfo.getCustomerSurname());
    }

    private void showMavRavLayout(int visibility) {
        mPaymentsManagerMAVRAV.setVisibility(visibility);
        mPaymentsManagerMAVRAV.setBillHolderName(Contants.getUserInfo.getCustomerName() + " "
                + Contants.getUserInfo.getCustomerSurname());
    }
    ...
    private View showPayee(int visibility) {
        ...
        if (showForm == FORM_RECENT_PAYMENT || showForm == FORM_PENDING_PAYMENT) {
            ...
            if (phone_payee_folders_cf.getVisibility() != View.GONE) {
                phone_payee_folders_cf.setVisibility(View.GONE);
                layout.findViewById(R.id.payee_list_info_tv).setVisibility(View.GONE);
            }
            ...
            if (getPaymentType() == PHONE_TOP_UP) {
                ...
                if (phone_payee_folders_cf.getVisibility() != visibility) {
                    ...
                    layout.findViewById(R.id.payee_list_info_tv).setVisibility(visibility);
                }
                if (payee_folders_cf.getVisibility() != View.GONE) {
                    payee_folders_cf.setVisibility(View.GONE);
                }
    ...
    private void invilidataPayees() {
        switch (getPaymentType()) {
            case BANK_TRANSFER:
                if (Contants.getRecipientListModel != null) {
                    bank_payee_cf_ad
                            .setDatas(Contants.getRecipientListModel.getBankRecipientList());
                    payee_folders_cf.setAdapter(bank_payee_cf_ad);
                }
                break;
            case TRANSFER_ENTRY:
            ...
            case CARD_TOP_UP:
                if (Contants.getRecipientListModel != null) {
                    card_payee_cf_ad
                            .setDatas(Contants.getRecipientListModel.getCardRecipientList());
                    payee_folders_cf.setAdapter(card_payee_cf_ad);
                }
                break;
            case PHONE_TOP_UP:
    ...
    private View showAmount(int visibility, double amount) {
        ...
        if (getPaymentType() == PHONE_TOP_UP) {
            ...
            if (amount == 0) {
                ...
                amount_et.setText(Integer.toString(m1));// Utils.formatMoney(m1,
                                                        // "", true, true,
                                                        // false, false, true));
        ...
        switch (getPaymentType()) {
            ...
            case CARD_TOP_UP:
                try {
                    ...
                    // amountStr = amountStr.replace(',', '.');
                    String amountStr = amount_et.getText().toString().trim() + "."
                            + amount_et2.getText().toString().trim();
    ...
    private int getPaymentType() {
        ...
        if (showForm != PaymentsManager.FORM_RECENT_PAYMENT && showForm != FORM_PENDING_PAYMENT) {
            ...
            if (paymentType == 0) {// 如果==0 带表account卡片一个都没有选
                account_folders_cf.setEnable(true);
            } else {
                account_folders_cf.setEnable(false);
            }
            ...
            switch (type) {
                ...
                case TYPE_UPDATE_BALANCE:
                    NewPaymentDataUtils.getBalance(activity,
                            ((List<AccountsForServiceModel>)postData).get(0).getAccounts(), null);
                    break;
                ...
                case TYPE_VERTIFY_CARD:
                    resultData = vertifyCard(mContext, getPayer().getAccountCode(),
    ...
    // private List<AccountsForServiceModel>
    // simTopUpAccounts=Contants.simTopUpAccounts;

    // private List<AccountsForServiceModel>
    // chargeAccounts=Contants.chargeAccounts;

    // private List<AccountsForServiceModel>
    // transferEntryPayeeAccounts=Contants.transferEntryPayeeAccounts;
    ...
    private AccountsModel getPayer() {
        if (showForm == PaymentsManager.FORM_RECENT_PAYMENT || showForm == FORM_PENDING_PAYMENT || showForm == PaymentsManager.FORM_ACCOUNT) {
            return curPayer;
        }
        ...
        if (showForm == PaymentsManager.FORM_RECENT_PAYMENT || showForm == FORM_PENDING_PAYMENT) {
            ...
            switch (getPaymentType()) {
                case BANK_TRANSFER:
                    ...
                    } else if (payee_folders_cf.getCount() - payee_folders_cf.getSelectedItemPosition() == 1) {
                    ...
                    if (br != null) {
                        ...
                        // DestaccountModel destAccount = new
                        // DestaccountModel();
                ...
                case TRANSFER_ENTRY:
                    if (entry_payee_cf_ad.getSelected() < 0) {
                        ...
                        if (entry_payee_cf_ad.getClickDisableId() >= 0
                                && index >= entry_payee_cf_ad.getClickDisableId()) {
                            ++index;
                        }
    ...
    private CheckTransferResponseModel validateBankTransfer() {
        ...
        String postData = CheckTransferJson.CheckTransferReportProtocal(
                ...
                dateString, destAccount, Contants.publicModel, "1000", transferId);
    ...
    private String post(String r) {
        HttpConnector httpConnector = new HttpConnector();
        return httpConnector.requestByHttpPost(Contants.mobile_url, r, activity);
    }
    ...
    private Object validate() {
        switch (getPaymentType()) {
            ...
            case PRECOMPILED_BILL:
                ...
                checkBillPaymentValue.setDueDate(TimeUtil.getDateString(System.currentTimeMillis(),
            ...
            case BLANK_BILL:
                ...
                checkBillPaymentValue
                        .setPostalAccount(mPaymentsManagerBlankBill.getAccountNumber());
                ...
                checkBillPaymentValue.setPurposeDescription(mPaymentsManagerBlankBill
                        .getDescription());
                checkBillPaymentValue.setCurrency("$");
                if (showForm == FORM_PENDING_PAYMENT && mPendingTransferModel != null) {
                    checkBillPaymentValue.setTransferId(mPendingTransferModel.getTransferId());
                ...
                checkBillPaymentValue.setDueDate(TimeUtil.getDateString(System.currentTimeMillis(),
                        TimeUtil.dateFormat2));
                json = CheckBillPaymentJson.CheckBillPaymentReportProtocal(Contants.publicModel,
                        checkBillPaymentValue);
                json = post(json);
                mCheckBillPaymentResponseModel = CheckBillPaymentJson
                        .ParseCheckBillPaymentResponse(json);
            ...
            case MAV_RAV:
                ...
                checkBillPaymentValue
                        .setPurposeDescription(mPaymentsManagerMAVRAV.getDescription());
                checkBillPaymentValue.setCurrency("$");
                if (showForm == FORM_PENDING_PAYMENT && mPendingTransferModel != null) {
                    checkBillPaymentValue.setTransferId(mPendingTransferModel.getTransferId());
    ...
    private Object confirm() {
        ...
        switch (getPaymentType()) {
            ...
            case PRECOMPILED_BILL:
                CheckBillPaymentValueModel mCheckBillPaymentValueModel = mCheckBillPaymentResponseModel
                ...
                mCheckBillPaymentValueModel.setPostalAccount(mPaymentsManagerBill
                        .getAccountNumber());
                mCheckBillPaymentValueModel.setCurrency(Contants.COUNTRY);
                mCheckBillPaymentValueModel.setTransferCheck(Contants.isNewPaymentsUpdate);
                NewPayee.BillHolder mBillHolder = mPaymentsManagerBill.getHolder();
                if (mBillHolder != null) {
                ...
                String json = InsertBillPaymentJson.InsertBillPaymentReportProtocal(
                        Contants.publicModel,
                        mCheckBillPaymentResponseModel.getCheckBillPaymentValueModel(),
                        generateOtp.getOtpKeySession(), generatePin(), Contants.OTP_CHANNEL_MAIL);
                HttpConnector httpConnector = new HttpConnector();
                json = httpConnector.requestByHttpPost(Contants.mobile_url, json, activity);
                return InsertBillPaymentJson.ParseCheckBillPaymentResponse(json);
            case BLANK_BILL:
                mCheckBillPaymentValueModel = mCheckBillPaymentResponseModel
                        .getCheckBillPaymentValueModel();
                mCheckBillPaymentValueModel.setBillNumber(mPaymentsManagerBlankBill
                        .getAccountNumber());
                mCheckBillPaymentValueModel.setBeneficiaryName(mPaymentsManagerBlankBill
                        .getPayableTo());
                mCheckBillPaymentValueModel.setPurposeDescription(mPaymentsManagerBlankBill
                        .getDescription());
                mCheckBillPaymentValueModel.setBillType(mPaymentsManagerBlankBill.getType());
                mCheckBillPaymentValueModel.setAccountCode(accountsModel.getAccountCode());
                mCheckBillPaymentValueModel.setPostalAccount(mPaymentsManagerBlankBill
                        .getAccountNumber());
                mCheckBillPaymentValueModel.setCurrency(Contants.COUNTRY);
                mCheckBillPaymentValueModel.setTransferCheck(Contants.isNewPaymentsUpdate);
                mBillHolder = mPaymentsManagerBlankBill.getHolder();
                ...
                json = InsertBillPaymentJson.InsertBillPaymentReportProtocal(Contants.publicModel,
                        mCheckBillPaymentValueModel, generateOtp.getOtpKeySession(), generatePin(),
            ...
            case MAV_RAV:
                ...
                mCheckBillPaymentValueModel.setBeneficiaryName(mPaymentsManagerMAVRAV
                        .getPayableTo());
                mCheckBillPaymentValueModel.setPurposeDescription(mPaymentsManagerMAVRAV
                        .getDescription());
                mCheckBillPaymentValueModel.setBillType(mPaymentsManagerMAVRAV.getType());
                mCheckBillPaymentValueModel.setAccountCode(accountsModel.getAccountCode());
                mCheckBillPaymentValueModel.setPostalAccount(mPaymentsManagerMAVRAV
                        .getAccountNumber());
                mCheckBillPaymentValueModel.setCurrency(Contants.COUNTRY);
                mCheckBillPaymentValueModel.setTransferCheck(Contants.isNewPaymentsUpdate);
                mBillHolder = mPaymentsManagerMAVRAV.getHolder();
    ...
    private ResponsePublicModel confirmCardRecharge(CheckRechargeCardResponseModel checkRechargeCard) {
        ...
        String postData = InsertRechargeCardJson.InsertRechargeCardReportProtocal(
                ...
                tmp_InfoCardsModel.getTitle(), generatePin(), generateOtp.getOtpKeySession(),
            ...
            case R.id.proceed_btn:
                if (getPaymentStep() == R.id.new_payments_step1_rb) {
                    ...
                    if (curType == 0) {
                        ...
                        Toast.makeText(activity, R.string.pls_select_payer, Toast.LENGTH_LONG)
                                .show();
                        break;
                    }
                    ...
                    switch (curType) {
                        ...
                        case BANK_TRANSFER:
                            ...
                            if (generateAmount == 0) {
                                Toast.makeText(activity, R.string.missing_amount, Toast.LENGTH_LONG)
                                        .show();
                                break;
                            }
                            if (getPayee() == null) {
                                Toast.makeText(activity, R.string.pls_select_payee,
                                        Toast.LENGTH_LONG).show();
                                return;
                            }
                            if (generateDate() <= 0) {
                                Toast.makeText(activity, R.string.missing_date, Toast.LENGTH_LONG)
                                        .show();
                                break;
                            }
                    ...
                    if (pin_lin.getVisibility() != View.VISIBLE) {
                        ...
                        if (TextUtils.isEmpty(pin_et.getText())) {
                            Toast.makeText(activity, R.string.missing_pin, Toast.LENGTH_SHORT)
                                    .show();
                            return;
                        }
                        downloading_pd.show();
        ...
        switch (arg0.getId()) {
            ...
            case R.id.payee_folders_cf:
                ...
                if (payment == BANK_TRANSFER && arg2 == arg0.getCount() - 2) {
                    ...
                    startAddItem(NewPayee.NEW_TRANSFER_PAYEE, NewPayee.NEW_TRANSFER_PAYEE,
                            tmp_payee);
                    ...
                    startAddItem(NewPayee.NEW_CARD_PAYEE, NewPayee.NEW_CARD_PAYEE, getPayer()
    ...
    private void checkAmountView() {
        ...
        amount_et2.requestFocus();
        amount_et2.setFocusableInTouchMode(true);
        amount_et2.setSelection(amount_et2.getText().length());
        // setFoucs(amount_et2);
