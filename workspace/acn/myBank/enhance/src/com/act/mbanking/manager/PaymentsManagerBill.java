...
import com.act.mbanking.activity.NewPayee.BillHolder;
...
public class PaymentsManagerBill implements OnClickListener {
	...
	public PaymentsManagerBill(Context context) {
		...
		layout = (ViewGroup) layoutInflater.inflate(R.layout.new_payment_bill,
				null);
		initView();
	}
	...
	private void initView() {
		...
		bill_holder_ibtn = (ImageButton) layout
				.findViewById(R.id.bill_holder_ibtn);
		...
		account_number_et = (EditText) layout
				.findViewById(R.id.account_number_et);
		amount_doller_et = (EditText) layout
				.findViewById(R.id.amount_doller_et);
	...
	private void loadData() {
		String dataStr = GetPaymentTemplatesJson
				.GetPaymentTemplatesReportProtocal(Contants.publicModel);
		GetPaymentTempLatesResponseModel mGetPaymentTempLatesResponseModel = GetPaymentTemplatesJson
				.ParseGetPaymentTempLatesResponse(dataStr);
	}
	...
	public void setAmount(double amount) {
		try {
			...
			amount_doller_et.setText(Integer.toString(m1));// Utils.formatMoney(m1,
															// "", true, true,
															// false, false,
															// true));
	...
	public void onClick(View v) {
		...
		case R.id.bill_holder_ibtn:
			NewPayee.start((Activity) context,
					NewPayee.PRECOMPILED_BILL_HOLDER,
					NewPayee.PRECOMPILED_BILL_HOLDER, null);
			break;
		case R.id.capture_code_btn:
			ViewUtil.goToViewForResult((Activity) context,
					CaptureActivity.class, R.id.search, CaptureRequestCode);
			// ViewUtil.goToViewByArgu(context, CaptureActivity.class,
	...
	public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == CaptureRequestCode) {
			...
			if (TextUtils.isEmpty(scaString)) {
				...
				amount = scaString.substring(37, 46) + ","
						+ scaString.substring(46, 48);
				...
				try {
					...
					amount = scaString.substring(index, end - 2) + ","
							+ scaString.substring(end - 2, end);
					index = end;
					end = index + 1;
					length = Integer.parseInt(scaString.substring(index, end));
					index = end;
