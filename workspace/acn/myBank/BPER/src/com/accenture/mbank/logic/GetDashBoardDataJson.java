...
public class GetDashBoardDataJson {
    public static String getDashBoardDataReportProtocal(RequestPublicModel publicModel,
            int granularity, int dashboardType) {
        String result = null;
        try {
            ...
            getDashBoardDataObj.put("customerNumber", publicModel.getCustomerNumber());
    ...
    public static GetDashBoardDataResponseModel ParseGetDashBoardDataResponse(String json) {
        ...
        try {
            ...
            getDashBoardDataResponse.responsePublicModel.setResultCode(getDashBoardDataObj.getInt("resultCode"));
            getDashBoardDataResponse.responsePublicModel.setResultDescription(getDashBoardDataObj.optString("resultDescription"));
            if (getDashBoardDataResponse.responsePublicModel.getResultCode() != 0) {JSONObject eventManagementObj = getDashBoardDataObj.getJSONObject("eventManagement");
                getDashBoardDataResponse.responsePublicModel.eventManagement.setErrorCode(eventManagementObj.optString("errorCode"));
                getDashBoardDataResponse.responsePublicModel.eventManagement.setErrorDescription(eventManagementObj.optString("errorDescription"));
                return getDashBoardDataResponse;
            }

            getDashBoardDataResponse.responsePublicModel.setTransactionId(getDashBoardDataObj.optString("transactionId"));
            ...
            for (int i = 0; i < dashboards.length(); i++) {
                ...
                dashBoard.setAccountBalance(dashboards.getJSONObject(i).optDouble("accountBalance"));
                dashBoard.setAccountCode(dashboards.getJSONObject(i).optString("accountCode"));
                dashBoard.setAvailableBalance(dashboards.getJSONObject(i).optDouble("availableBalance"));
                dashBoard.setPersonalizedName(dashboards.getJSONObject(i).optString("personalizedName"));

                JSONArray dashboardDataArray = dashboards.getJSONObject(i).getJSONArray("dashboardDataList");
                ...
                for (int j = 0; j < dashboardDataArray.length(); j++) {
                    ...
                    dashboardData.setAccountBalance(dashboardDataArray.getJSONObject(j).optDouble("accountBalance"));
                    dashboardData.setDeposits(dashboardDataArray.getJSONObject(j).optDouble("deposits"));
                    dashboardData.setLastUpdate(dashboardDataArray.getJSONObject(j).optString("lastUpdate"));
                    dashboardData.setWithdrawals(dashboardDataArray.getJSONObject(j).optDouble("withdrawals"));
