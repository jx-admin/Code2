...
public class InsertCommunicationJson {
    ...
    public static final String CONTACT_MODE_EMAIL = "Tramite e-mail";

    public static final String CONTACT_MODE_PHONE = "Tramite telefono";

    public static final String CONTACT_MODE_BRANCH = "Presso una filiale";
    
    public static final String BRANCH_PHONE = "phone";
    
    public static final String BRANCH_EMAIL = "mail";
    ...
    public static String InsertCommunicationProtocol(final RequestPublicModel publicModel,
            ...
            final String contactTime, final String contactDate,String email,String phone, String channelBranch) {

        try {
            JSONObject jsonObj = new JSONObject();
            JSONObject insertCommunicationObj = new JSONObject();
            JSONObject userinfoObj = new JSONObject();
//            insertCommunicationObj.put("transactionId","");
...
//            insertCommunicationObj.put("applicationId", );
            insertCommunicationObj.put("token", publicModel.getToken());
            insertCommunicationObj.put("customerNumber", publicModel.getCustomerNumber());
...
//            insertCommunicationObj.put("operationId", );
            ...
            insertCommunicationObj.put("ndgCode", Contants.getUserInfo.getUserprofileHb().getNdgHB().getNdgCode());
            userinfoObj.put("name", userInfo.user_name);
            if (contactMode.equals(CONTACT_MODE_EMAIL) || (contactMode.equals(CONTACT_MODE_BRANCH) && channelBranch==InsertCommunicationJson.BRANCH_EMAIL)) {
                userinfoObj.put("email", email);
            }

            if (contactMode.equals(CONTACT_MODE_PHONE) || (contactMode.equals(CONTACT_MODE_BRANCH)  && channelBranch==InsertCommunicationJson.BRANCH_PHONE)) {
                userinfoObj.put("mobilePhoneNumber", phone);
    ...
    public static ResponsePublicModel ParseInsertCommunicationResponse(final String jsonString) {
        ...
        try {
            ...
            JSONObject insertCommunicationObj = jsonObject.getJSONObject("InsertCommunicationResponse");
            responsePublicModel.setResultCode(insertCommunicationObj.getInt("resultCode"));
            responsePublicModel.setResultDescription(insertCommunicationObj.optString("resultDescription"));
            if (responsePublicModel.getResultCode() != 0) {
            	JSONObject eventManagementObj = insertCommunicationObj.getJSONObject("eventManagement");
