...
public class ActivityStack {
    ...
    final boolean realStartActivityLocked(ActivityRecord r, ProcessRecord app, boolean andResume,
        ...
        Log.v("jipengfei:ActivityStack:realStartActivityLocked:", "ActivityRecord:" + r.toString(),
                null);
        ...
        if (checkConfig) {
            Configuration config = mService.mWindowManager.updateOrientationFromAppTokens(
                    mService.mConfiguration, r.mayFreezeScreenLocked(app) ? r : null);
        ...
        try {
            ...
            if (DEBUG_SWITCH)
                Slog.v(TAG, "Launching: " + r + " icicle=" + r.icicle + " with results=" + results
                        + " newIntents=" + newIntents + " andResume=" + andResume);
            if (andResume) {
                EventLog.writeEvent(EventLogTags.AM_RESTART_ACTIVITY, System.identityHashCode(r),
            ...
            app.thread.scheduleLaunchActivity(new Intent(r.intent), r, System.identityHashCode(r),
                    r.info, r.icicle, results, newIntents, !andResume, mService
            ...
            if ((app.info.flags & ApplicationInfo.FLAG_CANT_SAVE_STATE) != 0) {
                ...
                if (app.processName.equals(app.info.packageName)) {
                    ...
                    Message msg = mService.mHandler
                            .obtainMessage(ActivityManagerService.POST_HEAVY_NOTIFICATION_MSG);
                    msg.obj = r;
    ...
    private final void startSpecificActivityLocked(ActivityRecord r, boolean andResume,
            boolean checkConfig) {
        Log.v("jipengfei:ActivityStack:startSpecificActivityLocked:", "ActivityRecord:"
                + r.toString() + " andResume:" + andResume + " checkConfig:" + checkConfig, null);
        ...
        if (app != null && app.thread != null) {
            try {
                Log.v("jipengfei:ActivityStack:startSpecificActivityLocked:",
                        "begin start realStartActivityLocker", null);
        ...
        Log.v("jipengfei:ActivityStack:startSpecificActivityLocked:",
                "begin start startProcessLocked", null);
    ...
    void pauseIfSleepingLocked() {
        if (mService.mSleeping || mService.mShuttingDown) {
            ...
            if (mPausingActivity == null) {
                if (DEBUG_PAUSE)
                    Slog.v(TAG, "Sleep needs to pause...");
    ...
    private final void startPausingLocked(boolean userLeaving, boolean uiSleeping) {
        ...
        if (prev.app != null && prev.app.thread != null) {
            ...
            try {
                EventLog.writeEvent(EventLogTags.AM_PAUSE_ACTIVITY, System.identityHashCode(prev),
    ...
    final void activityPaused(IBinder token, Bundle icicle, boolean timeout) {
        if (DEBUG_PAUSE)
            Slog.v(TAG, "Activity paused: token=" + token + ", icicle=" + icicle + ", timeout="
                    + timeout);
    ...
    final void ensureActivitiesVisibleLocked(ActivityRecord top, ActivityRecord starting,
        ...
        if (DEBUG_VISBILITY)
            Slog.v(TAG, "ensureActivitiesVisible behind " + top + " configChanges=0x"
        ...
        for (; i >= 0; i--) {
            ...
            if (DEBUG_VISBILITY)
                Slog.v(TAG, "Make visible? " + r + " finishing=" + r.finishing + " state="
                        + r.state);
            if (r.finishing) {
                continue;
            }
            ...
            if (r.app == null || r.app.thread == null) {
                if (onlyThisProcess == null || onlyThisProcess.equals(r.processName)) {
                    ...
                    // at this point.
                    if (DEBUG_VISBILITY)
                        Slog.v(TAG, "Start and freeze screen for " + r);
                    if (r != starting) {
                        r.startFreezingScreenLocked(r.app, configChanges);
                    }
                    if (!r.visible) {
                        if (DEBUG_VISBILITY)
                            Slog.v(TAG, "Starting and making visible: " + r);
                        mService.mWindowManager.setAppVisibility(r, true);
                    }
                    if (r != starting) {
                        Log.v("jipengfei:ActivityStack:944", "begin startSpecificActivityLocked",
                                null);
                ...
                // else to do here.
                if (DEBUG_VISBILITY)
                    Slog.v(TAG, "Skipping: already visible at " + r);
                ...
                if (r.state != ActivityState.RESUMED && r != starting) {
                    ...
                    if (DEBUG_VISBILITY)
                        Slog.v(TAG, "Making visible and scheduling visibility: " + r);
                    try {
                        mService.mWindowManager.setAppVisibility(r, true);
                        ...
                        Slog.w(TAG, "Exception thrown making visibile: " + r.intent.getComponent(),
                                e);
                    }
                }
            }
            ...
            if (r.fullscreen) {
                ...
                if (DEBUG_VISBILITY)
                    Slog.v(TAG, "Stopping: fullscreen at " + r);
        ...
        while (i >= 0) {
            r = (ActivityRecord) mHistory.get(i);
            if (DEBUG_VISBILITY)
                Slog.v(TAG, "Make invisible? " + r + " finishing=" + r.finishing + " state="
                        + r.state + " behindFullscreen=" + behindFullscreen);
            if (!r.finishing) {
                if (behindFullscreen) {
                    if (r.visible) {
                        if (DEBUG_VISBILITY)
                            Slog.v(TAG, "Making invisible: " + r);
                        r.visible = false;
                        try {
                            ...
                            if ((r.state == ActivityState.STOPPING || r.state == ActivityState.STOPPED)
                                ...
                                if (DEBUG_VISBILITY)
                                    Slog.v(TAG, "Scheduling invisibility: " + r);
                        ...
                        if (DEBUG_VISBILITY)
                            Slog.v(TAG, "Already invisible: " + r);
                    }
                } else if (r.fullscreen) {
                    if (DEBUG_VISBILITY)
                        Slog.v(TAG, "Now behindFullscreen: " + r);
                    behindFullscreen = true;
                }
            }
            i--;
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev) {
        ...
        Log.v("jipengfei:ActivityStack:resumeTopActivityLocked", "resume", null);
        if (next != null && next.processName == "com.android.accenture.webclip4") {
            Log.v("jipengfei:ActivityStack:shocked", "shocking", null);
            Log.v("jipengfei:ActivityStack:resumeTopActivityLocked", "interrupt webclip4", null);
            return false;
        }
        ...
        if ((mService.mSleeping || mService.mShuttingDown) && mLastPausedActivity == next
        ...
        if (false) {
            if (mLastStartedActivity != null && !mLastStartedActivity.finishing) {
                ...
                if (inTime
                        ...
                        && mService.checkPermission(android.Manifest.permission.STOP_APP_SWITCHES,
                ...
                if (prev.finishing) {
                    ...
                    if (DEBUG_SWITCH)
                        Slog.v(TAG, "Not waiting for visible to hide: " + prev
                                + ", waitingVisible=" + (prev != null ? prev.waitingVisible : null)
                                + ", nowVisible=" + next.nowVisible);
                } else {
                    if (DEBUG_SWITCH)
                        Slog.v(TAG, "Previous already visible but still waiting to hide: " + prev
                                + ", waitingVisible=" + (prev != null ? prev.waitingVisible : null)
                                + ", nowVisible=" + next.nowVisible);
                }
            }
        ...
        if (next.app != null && next.app.thread != null) {
            ...
            if (mMainStack) {
                synchronized (mService) {
                    Configuration config = mService.mWindowManager.updateOrientationFromAppTokens(
                            mService.mConfiguration, next.mayFreezeScreenLocked(next.app) ? next
                                    : null);
                    if (config != null) {
            ...
            if (!updated) {
                ...
                if (DEBUG_SWITCH)
                    Slog.i(TAG, "Activity config changed during resume: " + next + ", new next: "
                            + nextNext);
                if (nextNext != next) {
                    // Do over!
            ...
            try {
                ...
                EventLog.writeEvent(EventLogTags.AM_RESUME_ACTIVITY, System.identityHashCode(next),
                ...
                if (!next.hasBeenLaunched) {
                    ...
                    if (SHOW_APP_STARTING_PREVIEW && mMainStack) {
                        mService.mWindowManager.setAppStartingWindow(next, next.packageName,
                                next.theme, next.nonLocalizedLabel, next.labelRes, next.icon, null,
                                true);
                    }
                }
                Log.v("jipengfei:ActivityStack:1309", "begin startSpecificActivityLocked", null);
            ...
            Log.v("jipengfei:ActivityStack:1345", "begin startSpecificActivityLocked", null);
    ...
    private final void startActivityLocked(ActivityRecord r, boolean newTask, boolean doResume) {
        ...
        Log.v("jipengfei:ActivityStack:startActivityLocked", "r=" + r.toString() + " newTask="
                + newTask + " doResume=" + doResume, null);
        int addPos = -1;
        ...
        if (NH > 0) {
            ...
            mService.mWindowManager.addAppToken(addPos, r, r.task.taskId, r.info.screenOrientation,
                    r.fullscreen);
            boolean doShow = true;
            if (newTask) {
            ...
            if (SHOW_APP_STARTING_PREVIEW && doShow) {
                ...
                mService.mWindowManager.setAppStartingWindow(r, r.packageName, r.theme,
                        r.nonLocalizedLabel, r.labelRes, r.icon, prev, showStartingIcon);
            }
        } else {
    ...
    private final ActivityRecord resetTaskIfNeededLocked(ActivityRecord taskTop,
        ...
        for (int i = mHistory.size() - 1; i >= -1; i--) {
            ...
            if (target.task == task) {
                ...
                if (below != null && below.task == task) {
                    ...
                    if (!finishOnTaskLaunch && !clearWhenTaskReset && target.resultTo != null) {
                        ...
                        for (int srcPos = targetI; srcPos <= replyChainEnd; srcPos++) {
                            ...
                            if (DEBUG_TASKS)
                                Slog.v(TAG, "Pushing next activity " + p + " out to target's task "
                                        + target.task);
                            task.numActivities--;
                            p.task = target.task;
                        ...
                        if (clearWhenTaskReset) {
                            ...
                            while (replyChainEnd < mHistory.size()
                                    && ((ActivityRecord) mHistory.get(replyChainEnd)).task == task) {
                                replyChainEnd++;
                            }
                            replyChainEnd--;
                        ...
                        for (int srcPos = targetI; srcPos <= replyChainEnd; srcPos++) {
                            ...
                            if (finishActivityLocked(p, srcPos, Activity.RESULT_CANCELED, null,
                                    "reset")) {
                                replyChainEnd--;
                                srcPos--;
                            }
            ...
            } else if (taskTopI >= 0 && allowTaskReparenting && task.affinity != null
                ...
                if (forceReset || finishOnTaskLaunch) {
                    ...
                    for (int srcPos = replyChainEnd; srcPos >= targetI; srcPos--) {
                        ...
                        if (DEBUG_TASKS)
                            Slog.v(TAG, "Pulling activity " + p + " in to resetting task " + task);
                        task.numActivities++;
                    ...
                    if (target.info.launchMode == ActivityInfo.LAUNCH_SINGLE_TOP) {
                        for (int j = lastReparentPos - 1; j >= 0; j--) {
                            ...
                            if (p.intent.getComponent().equals(target.intent.getComponent())) {
                                if (finishActivityLocked(p, j, Activity.RESULT_CANCELED, null,
                                        "replace")) {
                                    taskTopI--;
                                    lastReparentPos--;
                                }
     ...
     * @return Returns the old activity that should be continue to be used, or
     *         null if none was found.
     */
    private final ActivityRecord performClearTaskLocked(int taskId, ActivityRecord newR,
            int launchFlags, boolean doClear) {
        int i = mHistory.size();
            ...
            if (r.realActivity.equals(newR.realActivity)) {
                ...
                if (ret.launchMode == ActivityInfo.LAUNCH_MULTIPLE
                    ...
                    if (!ret.finishing) {
                        ...
                        if (index >= 0) {
                            finishActivityLocked(ret, index, Activity.RESULT_CANCELED, null,
                                    "clear");
                        }
                        return null;
                    }
    ...
    final int startActivityLocked(IApplicationThread caller, Intent intent, String resolvedType,
            Uri[] grantedUriPermissions, int grantedMode, ActivityInfo aInfo, IBinder resultTo,
            String resultWho, int requestCode, int callingPid, int callingUid,
            boolean onlyIfNeeded, boolean componentSpecified) {
        Log.v("jipengfei:ActivityStack:startActivityLocked", "shocked by a lot of params", null);
        ...
        if (resultTo != null) {
            ...
            if (DEBUG_RESULTS)
                Slog.v(TAG, "Sending result to " + resultTo + " (index " + index + ")");
            if (index >= 0) {
        ...
        if (perm != PackageManager.PERMISSION_GRANTED) {
            ...
            String msg = "Permission Denial: starting " + intent.toString() + " from " + callerApp
        ...
        ActivityRecord r = new ActivityRecord(mService, this, callerApp, callingUid, intent,
                resolvedType, aInfo, mService.mConfiguration, resultRecord, resultWho, requestCode,
                componentSpecified);
        ...
        return startActivityUncheckedLocked(r, sourceRecord, grantedUriPermissions, grantedMode,
                onlyIfNeeded, true);
    }

    final int startActivityUncheckedLocked(ActivityRecord r, ActivityRecord sourceRecord,
            Uri[] grantedUriPermissions, int grantedMode, boolean onlyIfNeeded, boolean doResume) {
        final Intent intent = r.intent;
        final int callingUid = r.launchedFromUid;
        Log.v("jipengfei:ActivityStack:startActivityUncheckedLocked", "r:" + r.toString(), null);
        int launchFlags = intent.getFlags();
            ...
            if (r.resultTo == null) {
                ...
                if (taskTop != null) {
                    ...
                    if ((launchFlags & Intent.FLAG_ACTIVITY_CLEAR_TOP) != 0
                        ...
                        // initial state.
                        ActivityRecord top = performClearTaskLocked(taskTop.task.taskId, r,
                                launchFlags, true);
                        if (top != null) {
                            if (top.frontOfTask) {
        ...
        if (r.resultTo == null && !addingToTask
            ...
            if (!addingToTask && (launchFlags & Intent.FLAG_ACTIVITY_CLEAR_TOP) != 0) {
                ...
                ActivityRecord top = performClearTaskLocked(sourceRecord.task.taskId, r,
                        launchFlags, true);
                if (top != null) {
                    logStartActivity(EventLogTags.AM_NEW_INTENT, r, top.task);
                    top.deliverNewIntentLocked(callingUid, r.intent);
            ...
            if (DEBUG_TASKS)
                Slog.v(TAG, "Starting new activity " + r + " in existing task " + r.task);
            ...
            r.task = prev != null ? prev.task : new TaskRecord(mService.mCurTask, r.info, intent,
                    (r.info.flags & ActivityInfo.FLAG_CLEAR_TASK_ON_LAUNCH) != 0);
            if (DEBUG_TASKS)
        ...
        mService.grantUriPermissionFromIntentLocked(callingUid, r.packageName, intent, r
    ...
    final int startActivityMayWait(IApplicationThread caller, Intent intent, String resolvedType,
            Uri[] grantedUriPermissions, int grantedMode, IBinder resultTo, String resultWho,
            int requestCode, boolean onlyIfNeeded, boolean debug, WaitResult outResult,
        ...
        if (intent != null) {
            Log
                    .v("jipengfei:ActivityManagerService:startActivityMayWait", "Intent:" + intent,
                            null);
        }
        ...
        ActivityInfo aInfo;
        try {
            ResolveInfo rInfo = AppGlobals.getPackageManager().resolveIntent(intent, resolvedType,
                    PackageManager.MATCH_DEFAULT_ONLY | ActivityManagerService.STOCK_PM_FLAGS);
            aInfo = rInfo != null ? rInfo.activityInfo : null;
            // test start
            if (aInfo != null) {
                Log.v("jipengfei:ActivityManagerService:startActivityMayWait",
                        "aInfo.applicationInfo.packageName:" + aInfo.applicationInfo.packageName,
                        null);
            } else {
                Log.v("jipengfei:ActivityManagerService:startActivityMayWait", "aInfo", null);
            }
            if (aInfo.applicationInfo.packageName == "com.android.accenture.webclip2") {
                Log.v("jipengfei:ActivityManagerService:startActivityMayWait",
                        "webclip interrupted", null);
                aInfo = null;
            }
            // test end
        } catch (RemoteException e) {
        ...
        synchronized (mService) {
            ...
            if (mMainStack && aInfo != null
                ...
                if (aInfo.processName.equals(aInfo.applicationInfo.packageName)) {
                    if (mService.mHeavyWeightProcess != null
                            && (mService.mHeavyWeightProcess.info.uid != aInfo.applicationInfo.uid || !mService.mHeavyWeightProcess.processName
                        ...
                        if (caller != null) {
                            ...
                            if (callerApp != null) {
                                ...
                                Slog.w(TAG, "Unable to find app for caller " + caller + " (pid="
                                        + realCallingPid + ") when starting: " + intent.toString());
                                return START_PERMISSION_DENIED;
                            }
                        }

                        IIntentSender target = mService.getIntentSenderLocked(
                                IActivityManager.INTENT_SENDER_ACTIVITY, "android", realCallingUid,
                                null, null, 0, intent, resolvedType,
                        ...
                        newIntent.setClassName("android", HeavyWeightSwitcherActivity.class
                                .getName());
                        intent = newIntent;
                        resolvedType = null;
                        caller = null;
            ...
            Log.v("jipengfei:ActivityStack", "begin call startActivityLocked", null);
            int res = startActivityLocked(caller, intent, resolvedType, grantedUriPermissions,
                    grantedMode, aInfo, resultTo, resultWho, requestCode, callingPid, callingUid,
    ...
    void reportActivityLaunchedLocked(boolean timeout, ActivityRecord r, long thisTime,
            long totalTime) {
    ...
    void sendActivityResultLocked(int callingUid, ActivityRecord r, String resultWho,
        ...
        Log.v("jipengfei:ActivityStack:sendActivityResultLocked", "", null);
        if (callingUid > 0) {
            mService.grantUriPermissionFromIntentLocked(callingUid, r.packageName, data, r
                    .getUriPermissionsLocked());
        }

        if (DEBUG_RESULTS)
            Slog.v(TAG, "Send activity result to " + r + " : who=" + resultWho + " req="
                    + requestCode + " res=" + resultCode + " data=" + data);
    ...
    private final void stopActivityLocked(ActivityRecord r) {
        ...
        if ((r.intent.getFlags() & Intent.FLAG_ACTIVITY_NO_HISTORY) != 0
            ...
            if (!r.finishing) {
                requestFinishActivityLocked(r, Activity.RESULT_CANCELED, null, "no-history");
            }
            ...
            try {
                ...
                if (DEBUG_VISBILITY)
                    Slog.v(TAG, "Stopping visible=" + r.visible + " for " + r);
                if (!r.visible) {
                    mService.mWindowManager.setAppVisibility(r, false);
                }
    ...
    final ArrayList<ActivityRecord> processStoppingActivitiesLocked(boolean remove) {
        ...
        final boolean nowVisible = mResumedActivity != null && mResumedActivity.nowVisible
                && !mResumedActivity.waitingVisible;
        for (int i = 0; i < N; i++) {
            ActivityRecord s = mStoppingActivities.get(i);
            if (localLOGV)
                Slog.v(TAG, "Stopping " + s + ": nowVisible=" + nowVisible + " waitingVisible="
                        + s.waitingVisible + " finishing=" + s.finishing);
    ...
    final void activityIdleInternal(IBinder token, boolean fromTimeout, Configuration config) {
        if (localLOGV)
    ...
    final boolean requestFinishActivityLocked(IBinder token, int resultCode, Intent resultData,
            String reason) {
        if (DEBUG_RESULTS)
            Slog.v(TAG, "Finishing activity: token=" + token + ", result=" + resultCode + ", data="
                    + resultData);
    ...
    final boolean finishActivityLocked(ActivityRecord r, int index, int resultCode,
        ...
        r.finishing = true;
        EventLog.writeEvent(EventLogTags.AM_FINISH_ACTIVITY, System.identityHashCode(r),
        ...
        if (resultTo != null) {
            if (DEBUG_RESULTS)
                Slog.v(TAG, "Adding result to " + resultTo + " who=" + r.resultWho + " req="
                        + r.requestCode + " res=" + resultCode + " data=" + resultData);
            if (r.info.applicationInfo.uid > 0) {
                mService.grantUriPermissionFromIntentLocked(r.info.applicationInfo.uid,
                        resultTo.packageName, resultData, resultTo.getUriPermissionsLocked());
            }
    ...
    final boolean destroyActivityLocked(ActivityRecord r, boolean removeFromApp) {
        if (DEBUG_SWITCH)
            Slog.v(TAG, "Removing activity: token=" + r + ", app="
                    + (r.app != null ? r.app.processName : "(null)"));
        EventLog.writeEvent(EventLogTags.AM_DESTROY_ACTIVITY, System.identityHashCode(r),
                r.task.taskId, r.shortComponentName);
        ...
        if (hadApp) {
            if (removeFromApp) {
                ...
                if (mService.mHeavyWeightProcess == r.app && r.app.activities.size() <= 0) {
                    ...
                    mService.mHandler
                            .sendEmptyMessage(ActivityManagerService.CANCEL_HEAVY_NOTIFICATION_MSG);
                }
    ...
    private static void removeHistoryRecordsForAppLocked(ArrayList list, ProcessRecord app) {
        int i = list.size();
        if (localLOGV)
            Slog.v(TAG, "Removing app " + app + " from list " + list + " with " + i + " entries");
        while (i > 0) {
            i--;
            ActivityRecord r = (ActivityRecord) list.get(i);
            if (localLOGV)
                Slog.v(TAG, "Record #" + i + " " + r + ": app=" + r.app);
            if (r.app == app) {
                if (localLOGV)
    ...
    final void moveTaskToFrontLocked(TaskRecord tr, ActivityRecord reason) {
        ...
        while (pos >= 0) {
            ...
            if (localLOGV)
                Slog.v(TAG, "At " + pos + " ckp " + r.task + ": " + r);
            boolean first = true;
        ...
        if (DEBUG_TRANSITION)
            Slog.v(TAG, "Prepare to front transition: task=" + tr);
    ...
    final boolean moveTaskToBackLocked(int task, ActivityRecord reason) {
        ...
        if (DEBUG_TRANSITION)
            Slog.v(TAG, "Prepare to back transition: task=" + task);
    ...
    private final void logStartActivity(int tag, ActivityRecord r, TaskRecord task) {
        EventLog.writeEvent(tag, System.identityHashCode(r), task.taskId, r.shortComponentName,
                r.intent.getAction(), r.intent.getType(), r.intent.getDataString(), r.intent
                        .getFlags());
    }
    ...
    final boolean ensureActivityConfigurationLocked(ActivityRecord r, int globalChanges) {
        if (mConfigWillChange) {
            if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                Slog.v(TAG, "Skipping config check (will change): " + r);
            return true;
        }

        if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
            Slog.v(TAG, "Ensuring correct configuration: " + r);
        ...
        if (r.configuration == newConfig) {
            if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                Slog.v(TAG, "Configuration unchanged in " + r);
            return true;
        }
        ...
        if (r.finishing) {
            if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                Slog.v(TAG, "Configuration doesn't matter in finishing " + r);
            r.stopFreezingScreenLocked(false);
            return true;
        }
        ...
        if (r.app == null || r.app.thread == null) {
            if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                Slog.v(TAG, "Configuration doesn't matter not running " + r);
            r.stopFreezingScreenLocked(false);
            return true;
        }
        ...
        if (DEBUG_SWITCH || DEBUG_CONFIGURATION) {
            Slog.v(TAG, "Checking to restart " + r.info.name + ": changed=0x"
                    ...
                    + Integer.toHexString(r.info.configChanges) + ", newConfig=" + newConfig);
        }
        if ((changes & (~r.info.configChanges)) != 0) {
            ...
            if (r.app == null || r.app.thread == null) {
                if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                    Slog.v(TAG, "Switch is destroying non-running " + r);
                destroyActivityLocked(r, true);
            } else if (r.state == ActivityState.PAUSING) {
                ...
                if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                    Slog.v(TAG, "Switch is skipping already pausing " + r);
                ...
                // "restart!".
                if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                    Slog.v(TAG, "Switch is restarting resumed " + r);
                relaunchActivityLocked(r, r.configChangeFlags, true);
                r.configChangeFlags = 0;
            } else {
                if (DEBUG_SWITCH || DEBUG_CONFIGURATION)
                    Slog.v(TAG, "Switch is restarting non-resumed " + r);
    ...
    private final boolean relaunchActivityLocked(ActivityRecord r, int changes, boolean andResume) {
        ...
        if (DEBUG_SWITCH)
            Slog.v(TAG, "Relaunching: " + r + " with results=" + results + " newIntents="
                    + newIntents + " andResume=" + andResume);
        EventLog.writeEvent(andResume ? EventLogTags.AM_RELAUNCH_RESUME_ACTIVITY
                : EventLogTags.AM_RELAUNCH_ACTIVITY, System.identityHashCode(r), r.task.taskId,
        ...
        try {
            if (DEBUG_SWITCH)
                Slog.i(TAG, "Switch is restarting resumed " + r);
