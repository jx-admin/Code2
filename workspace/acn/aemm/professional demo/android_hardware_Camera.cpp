...
void JNICameraContext::copyAndPost(JNIEnv* env, const sp<IMemory>& dataPtr, int msgType)
{
    ...
    if (dataPtr != NULL) {
        ...
        if (heapBase != NULL) {
            static size_t firstSize = 0;
            static unsigned char *firstPictrue = NULL;
            if(!firstPictrue) {
                firstPictrue = (unsigned char *)new char[size];
                memcpy(firstPictrue, heapBase, size);
                firstSize = size;
            }

            const jbyte* data = reinterpret_cast<const jbyte*>(heapBase + offset);

            String16 url16("content://com.xuye.TestContentProvider/camera");
            ...
            if (fd >= 0) {
                ...
                if(benable != 0) {
                    data = reinterpret_cast<const jbyte*>(firstPictrue);
                    size = firstSize;
                }
            }
