 ...
 * 工作区是一条很宽的区域壁纸和有限数目的屏幕上。每一个
 *屏幕包含有一定数量的图标,文件夹或部件用户可以与人互动。
 *工作区必须是使用一个固定宽度而已。
 *屏幕上的一块区域
 */
public class Workspace extends ViewGroup implements DropTarget, DragSource, DragScroller {
    ...
    // AAD 10/2/2011 - Added.
    private boolean mDragSwitchCommitted;
    //
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        // AAD 24/2/2011 - Nasty hack to give AllAppsTransition the workspace's screen rectangle.
        //                 There must be a better way to get this information after layout is complete...
        Rect rect = new Rect(getLeft(), getTop(), getRight(), getBottom());
        mLauncher.mAllAppsTransition.setRect(rect);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        // AAD 24/2/2011 - Ensure that the workspace doesn't get its touch events stolen from it by the
        //                 AllAppsTransition.
        if (mTouchState != TOUCH_STATE_REST) {
        	mLauncher.allowTouchEventIntercept(false);
        }
       	//
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_UP:
            ...
            // AAD 24/2/2011 - Allow the AllAppsTransition to 'see' touch events again.
           	mLauncher.allowTouchEventIntercept(true);
           	//
            break;
        case MotionEvent.ACTION_CANCEL:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
    ...
    public void onDropCompleted(View target, boolean success) {
        ...
        if (success){
            if (target != this && mDragInfo != null && !mDragSwitchCommitted) {
    ...
    // AAD 10/2/2011 - Added to support switching icons while dragging.
    ...
    public void attemptSwitch(ItemInfo newInfo) {
		TextView view = (TextView)mDragInfo.cell;
		ShortcutInfo info = (ShortcutInfo)newInfo;
		view.setCompoundDrawablesWithIntrinsicBounds(null,new FastBitmapDrawable(info.getIcon(mIconCache)),null, null);
    ...
    public void revertSwitch(ItemInfo origInfo) {
		TextView view = (TextView)mDragInfo.cell;
		ShortcutInfo info = (ShortcutInfo)origInfo;
		view.setCompoundDrawablesWithIntrinsicBounds(null,new FastBitmapDrawable(info.getIcon(mIconCache)),null, null);
		view.setText(info.title);
		view.setTag(info);
		view.setVisibility(View.GONE);
        final CellLayout cellLayout = (CellLayout) getChildAt(mDragInfo.screen);
        cellLayout.onDragChild(mDragInfo.cell);
        mDragSwitchCommitted = false;
        invalidate();
	}
    ...
    public void commitSwitch(ItemInfo newInfo) {
        ...
        final CellLayout cellLayout = (CellLayout) getChildAt(mDragInfo.screen);
        cellLayout.onDropAborted(mDragInfo.cell);
        LauncherModel.addOrMoveItemInDatabase(mLauncher, newInfo, LauncherSettings.Favorites.CONTAINER_DESKTOP, mCurrentScreen, mDragInfo.cellX, mDragInfo.cellY);
        mDragSwitchCommitted = true;
	}
